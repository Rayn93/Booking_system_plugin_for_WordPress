...
<!--                <input type="text" name="entry[event_turn]" id="gertis_event_turn" value="--><?php //echo $Guest->getField('event_turn'); ?><!--"/>-->
                <select name="entry[event_turn]" id="gertis_event_turn">
                    ...
                    <?php elseif(!empty($EventList)): ?>
                        ...
                        <!--Instrukcja poprawiająca błąd edycji uczestnika kiedy wszystkie miejsca na dany turnus są już zajete -->
                        <?php if (!(in_array($Guest->getField('event_turn'), array_column($EventList, 'event_turn')))): ?>

                            <?php $Model = new Gertis_BookingSystem_Model(); ?>

                            <option value="<?php echo $Guest->getField('event_turn'); ?>" selected="selected">
                                <?php echo $Guest->getField('event_turn').' (termin: '.$Model->getEventDate($Guest->getField('event_turn')).' )' ; ?>
                            </option>
