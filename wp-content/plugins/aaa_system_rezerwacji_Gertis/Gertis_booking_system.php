        ...
        switch ($view) {
            ...
            case 'event-form':
                ...
                if($eventid > 0){
                    ...
                    $oldEventCode = $EventEntry->getField('event_turn');
                ...
                if($action == 'save' && $request->isMethod('POST') && $_POST['entry']){
                    ...
                    if(check_admin_referer($this->action_token)){
                        ...
                        $newEventCode = $EventEntry->getField('event_turn');

                        if($EventEntry->validate()){

                            //Sprawdzenie czy kod turnusu został zmieniony, jeżeli tak to zmieniamy nazwę tego turnusu dla wszystkich zapisanych uczestników
                            if(isset($oldEventCode) && $oldEventCode != $newEventCode){
                                $this->model->changeEventCodeforMembers($oldEventCode, $newEventCode);
                            }
        ...
        switch ($view) {
            ...
            case 'email-to-guests':
                ...
                if(isset($event_turn) && !isset($action)){
					...
					$guest_mails_array = explode(',', $_POST['guests_emails'] );
					
					 foreach ($guest_mails_array as $value) {
						$headers[] = 'Bcc: '.$value;
					}
...
//                    }
    ...
    function getAdminEmail(){
		...
		//Maile na sztywno
		$admin_emails[]= 'biuro@gertis.pl';
		$admin_emails[]= 'brozka@gertis.pl';
    ...
    public function sendEmail($type_message, $to, $mail_params = array() ){
        ...
        $link_to_members = get_site_url().'/wp-admin/admin.php?page=gertis-book-system-guests';
